...
- **DCR (RFC 7591 dynamic client registration).** Deferred to v1.1. v1 ships the
  pre-registered client (paste id/secret from `init`). Claude's connector UI
  accepts a manually-entered Client ID/Secret in Advanced settings, so this works
  without DCR. **Verify before committing:** that omitting `registration_endpoint`
  from the AS metadata makes the current Claude connector flow fall back cleanly
  to manual entry (rather than erroring). If it doesn't fall back, DCR moves into
  v1. Add when: the manual-paste friction is real, or the fallback check fails.
...
| owner password | `DOORMAN_OWNER_PASSWORD` | — | omit ⇒ consent auto-approves (loud warning) |
...
## 8. Open decisions for Sebastiaan

1. **DCR in v1 or v1.1?** Recommend **v1.1**, gated on the fallback-check above.
2. **Built-in TLS?** Recommend **fast-follow**, cargo-feature-gated.
3. **Consent default:** recommend **require owner password**, opt-out via unset with a loud warning.
4. **License:** MIT/Apache-2.0 dual (Rust default) — assumed unless you object.
5. **Public repo now or after M1?** Recommend making it public once M1 lands
   (beta-secure) rather than shipping the picnic-named seed publicly on day one.
